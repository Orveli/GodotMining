...
func _erase_body(body: RigidBodyData, grid: PackedByteArray, seed: PackedByteArray, w: int, h: int) -> void:
	# PLANEETTA + PERF: SoA-flat-cache (P4) + x-wrap (planet). Maailma-x wrappaa modulo-W
	# (kappale ei katoa saumassa), vain y-rajat rajaavat.
	...
	for k in fox.size():
		var wy := foy[k] + py
		if wy >= 0 and wy < h:
			var idx := wy * w + PlanetGeom.wrap_x(fox[k] + px, w)
			if body_map[idx] == bid:
				grid[idx] = 0
				seed[idx] = 0
				body_map[idx] = 0
...
func _write_body(body: RigidBodyData, grid: PackedByteArray, seed: PackedByteArray, w: int, h: int) -> int:
	...
	for k in fox.size():
		var wy := foy[k] + py
		if wy >= 0 and wy < h:
			var wx := PlanetGeom.wrap_x(fox[k] + px, w)
			var idx := wy * w + wx
			var cur := grid[idx]
			if cur == 0 or _is_liquid(cur):
				...
				if _is_liquid(cur):
					...
					for disp in [Vector2i(-1, 0), Vector2i(1, 0), Vector2i(0, -1), Vector2i(-1, -1), Vector2i(1, -1)]:
						# PLANEETTA: naapuri wrappaa x:ssa (wx on jo wrapattu), y-rajat sailyvat.
						var ny: int = wy + disp.y
						if ny >= 0 and ny < h:
							var nidx := ny * w + PlanetGeom.wrap_x(wx + disp.x, w)
...
func _check_env_collision(body: RigidBodyData, grid: PackedByteArray, w: int, h: int) -> bool:
	# PLANEETTA + PERF: SoA-rot-cache (P4) + x-wrap (planet). x-reunalla ei ole seinaa
	# (wrappaa); vain y-rajat = tormays.
	...
	for i in rox.size():
		...
		if wy < 0 or wy >= h:
			return true
		var mat := grid[wy * w + PlanetGeom.wrap_x(rox[i] + px, w)]
...
func _find_env_collision(body: RigidBodyData, grid: PackedByteArray, w: int, h: int) -> CollisionResult:
	...
	for i in rox.size():
		...
		# PLANEETTA + PERF: SoA-rot (P4) + x-wrap (planet). x-reunalla ei ole seinaa
		# (wrappaa); vain y-rajat = tormays (seina).
		if wy < 0 or wy >= h:
			colliding = true
			if wy < 0: accumulated_normal += Vector2(0, 1)
			else: accumulated_normal += Vector2(0, -1)
		else:
			var gx := PlanetGeom.wrap_x(wx, w)
			var idx := wy * w + gx
			...
			if hit_mat != 0 and not _is_liquid(hit_mat):
				...
				if grid[wy * w + PlanetGeom.wrap_x(wx - 1, w)] == 0: local_normal.x -= 1.0
				if grid[wy * w + PlanetGeom.wrap_x(wx + 1, w)] == 0: local_normal.x += 1.0
